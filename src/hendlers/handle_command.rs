...
use crate::bot::{ADMIN_ID, Command, MyBot, State};
use crate::db::DatabaseHandler;
use crate::SantaBot;
use crate::types::User;

pub async fn handle_command(db: DatabaseHandler, bot: SantaBot, msg: Message) -> ResponseResult<()> {
    ...
    match BotCommands::parse(msg.text().unwrap(), "") {
        ...
        Ok(Command::Help) => { my_bot.send_help(bot, msg).await?; }
        _ => {}
    }
    return Ok(());
}
